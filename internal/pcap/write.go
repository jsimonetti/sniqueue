// taken from github.com/google/gopacket/gopcap/write.go
package pcap
...
func (w *Writer) writePacketHeader(len uint32) error {
	...
	binary.LittleEndian.PutUint32(w.buf[8:12], len)
	binary.LittleEndian.PutUint32(w.buf[12:16], len)
...
func (w *Writer) WritePacket(data []byte) error {
	if err := w.writePacketHeader(uint32(len(data))); err != nil {
